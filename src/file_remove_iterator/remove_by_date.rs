...
fn remove_by_date(path: &str, time: &str, older: bool) -> Result<(), Error> {
    ...
    match time_remove {
        ...
        Err(ch) => Err(Error::new(ErrorKind::InvalidInput, format!("unknow token '{}' in time specifier '{}'", ch, time)))
    }
...
impl<'a> Parser<'a> {
    ...
    fn get_status(token: char, status: Status) -> Status {
        match status {
            Status::Init => {if token.is_digit(10) {Status::Number} else {Status::Error}},
            Status::Number => {if token.is_digit(10)
                {Status::Number}
...
mod test {
    ...
    fn test_time_parse() {
        ...
        assert_eq!(TimeRemove::parse_time_str("1y1M1w"),Ok(Duration::new(34732800, 0)));
        // Error
        assert_eq!(TimeRemove::parse_time_str("1yy"), Err('y'));
        // 67 day + 1 year -> the order does not metter
        assert_eq!(TimeRemove::parse_time_str("67d1y"), Ok(Duration::new(37324800, 0)));
        // 1 year + 1 month + 1 week + 1 day + 1 hour + 1 minute + 1 second
        assert_eq!(TimeRemove::parse_time_str("1y1M1w1d1h1m1s"), Ok(Duration::new(34822861, 0)));
    }
    ...
    fn test_remove_wrong_time_format() {
        ...
        match stat {
            ...
            Err(err) => {
                ...
                assert_eq!(error::Error::description(&err), "unknow token 'e' in time specifier '3w4e'")
            }
        }
    }
